 ...
 */
export interface OrderNode extends BaseASTNode {
    type: 'order';
    name: string;
}
...
/**
 * Expression context with variable values
 */
...
export default class ExpressionEvaluator {
    ...
    evaluate(expression: string, context: ExpressionContext): number | null {
        ...
        try {
            ...
            if (errorMessage.includes('Division by zero')) {
                Logger.warn(`Expression evaluation warning: ${errorMessage}`);
                return null;
            }
            ...
            // Wrap other errors
            throw ErrorFactory.expressionError(expression, error as Error);
        }
    }
     ...
     */
    private _evaluateNode(node: ASTNode, context: ExpressionContext): number {
        switch (node.type) {
            case 'number':
                return node.value;
     ...
     * @private
     * @param node - AST node
     * @param dependencies - Set to collect dependencies into
     */
    private _collectDependencies(node: ASTNode, dependencies: Set<string>): void {
        switch (node.type) {
            case 'variable':
