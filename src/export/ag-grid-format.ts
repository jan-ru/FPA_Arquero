/**
 * ag-grid-format.ts - Functions that compute cell styles for the ag-Grid
 *
 ...
 *
 * These styles are then mapped to Excel formats by excel-format.ts
 */
 ...
 * @returns Style object with properties: font, fill, alignment, border, numFmt
 */
export function getCellStyle(params: CellParams): CellStyle {
    if (!params.data) {
 ...
 * @returns Style object for header cell
 */
export function getHeaderStyle(headerName: string, field?: string): CellStyle {
    return {
        font: {
            name: 'Calibri',
            size: 11,
 ...
 */
function getDefaultStyle(): CellStyle {
    return {
        font: {
            name: 'Calibri',
            size: 11,
 ...
 * @param rowType - Type of row (total, metric, group, detail, spacer)
 * @returns Height in points
 */
export function getRowHeight(rowType: string): number {
    switch (rowType) {
        case 'total':
            return 24; // 32px
        case 'metric':
 ...
 * @param field - Column field name
 * @param headerName - Column header text
 * @returns Width in characters (Excel units)
 */
export function getColumnWidth(field: string, headerName?: string): number {
    if (field === 'label') {
