...
import { EXPORT_CONFIG } from '../constants.ts';
...
class ExportHandler {
    ...
    formatCellsAsNumbers(row: any, startCol: number, endCol: number): void {
    ...
    styleMetricRow(row: any, startCol: number, endCol: number, highlighted: boolean = true): void {
        row.font = { bold: true };
        if (highlighted) {
            row.fill = {
                type: 'pattern',
    ...
    tableToArrays(table: ArqueroTable, includeHeaders: boolean = true): any[][] {
        const data: any[][] = [];
        ...
        table.objects().forEach((row: any) => {
            const rowArray = table.columnNames().map((col: string) => row[col]);
            data.push(rowArray);
        });
    ...
    async exportAllStatements(statements: Statements): Promise<void> {
        ...
        try {
            ...
            for (const [name, data] of Object.entries(statements)) {
                ...
                const categoryStartRows: Record<string, number> = {}; // Track where each category starts for formulas
                const categoryEndRows: Record<string, number> = {}; // Track where each category ends for formulas
