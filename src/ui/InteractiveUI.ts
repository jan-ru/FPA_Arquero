...
interface StatementMetrics {
    ...
    operatingIncome?: { [year: string]: number };
    netIncome?: { [year: string]: number };
    startingCash?: { [year: string]: number };
    netChange?: { [year: string]: number; variance?: number };
    endingCash?: { [year: string]: number };
}
...
class InteractiveUI {
    private currentStatement: StatementData | null = null;
    private currentStatementType: string | null = null;
    ...
    renderPeriodDropdown(id: string, currentValue: string, years: readonly string[] = APP_CONFIG.YEARS): string {
    ...
    renderTableRow(
        ...
        show2024: boolean,
        show2025: boolean,
        variance1Mode: string,
        variance2Mode: string,
    ...
    renderStatement(statementData: StatementData, statementType: string, containerId: string): void {
        ...
        const headerYear1Checkbox = document.getElementById(`show-${year1}-header`) as HTMLInputElement | null;
        const headerYear2Checkbox = document.getElementById(`show-${year2}-header`) as HTMLInputElement | null;
        ...
        const variance1Dropdown = document.getElementById('variance-1-header') as HTMLSelectElement | null;
        const variance2Dropdown = document.getElementById('variance-2-header') as HTMLSelectElement | null;
        ...
        const currentPeriodYear1 = (document.getElementById(`period-${year1}-header`) as HTMLSelectElement)?.value || `${year1}-all`;
        const currentPeriodYear2 = (document.getElementById(`period-${year2}-header`) as HTMLSelectElement)?.value || `${year2}-all`;

        // Get current detail level selection
        const detailLevelDropdown = document.getElementById('detail-level-header') as HTMLSelectElement | null;
        ...
        if (statementType === UI_STATEMENT_TYPES.BALANCE_SHEET) {
            totals.forEach((row: any) => {
                ...
                if (CATEGORY_DEFINITIONS.ASSETS.some((cat: string) => categoryLower.includes(cat))) {
        ...
        if (statementType === UI_STATEMENT_TYPES.CASH_FLOW && statementData.metrics) {
            ...
            if (metrics.netChange) {
                const ncVariance = metrics.netChange.variance || 0;
        ...
        const periodYear1HeaderDropdown = container.querySelector(`#period-${year1}-header`) as HTMLSelectElement | null;
        const periodYear2HeaderDropdown = container.querySelector(`#period-${year2}-header`) as HTMLSelectElement | null;
        ...
        if (periodYear1HeaderDropdown) {
            periodYear1HeaderDropdown.addEventListener('change', (e) => {
                ...
                if (this.currentStatementType) {
                    ...
                    (window as any).uiController.generateAndDisplayStatement(this.currentStatementType);
                }
            });
        }
        ...
        const variance1HeaderDropdown = container.querySelector('#variance-1-header') as HTMLSelectElement | null;
        const variance2HeaderDropdown = container.querySelector('#variance-2-header') as HTMLSelectElement | null;
        ...
        const detailLevelHeaderDropdown = container.querySelector('#detail-level-header') as HTMLSelectElement | null;
    ...
    enableDrillDown(container: HTMLElement): void {
    ...
    enableFiltering(container: HTMLElement): void {
    ...
    // Show tooltip
    showTooltip(element: HTMLElement, data: TooltipData): void {
