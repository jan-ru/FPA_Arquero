...
const tests: TestSuite[] = [
    {
        name: 'Unit Tests',
        script: 'test/unit',
        description: 'Run all unit tests for modules (CategoryMatcher, VarianceCalculator, DataStore)'
    },
    {
...
async function runTest(test: TestSuite): Promise<{ passed: boolean; exitCode: number }> {
    ...
    try {
        // Check if this is a unit test directory
        const isUnitTest = test.script.includes('test/unit');

        const command = new Deno.Command('deno', {
            args: isUnitTest
                ? ['test', '--allow-read', '--quiet', test.script]
                : ['run', '--allow-read', '--allow-env', '--allow-sys', test.script],
            stdout: 'inherit',
            stderr: 'inherit'
        });
